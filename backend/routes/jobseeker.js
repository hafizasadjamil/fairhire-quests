  ...
  async (req, res) => {
    try {
      ...
      if (req.files?.resume?.length) {
        ...
        resumeFilePath = `${baseUrl}/uploads/${resumeFile.filename}`;
        updates.resumeUrl = resumeFilePath;
      }
      ...
      const updatedUser = await User.findByIdAndUpdate(userId, updates, {
        ...
        "name email phone bio skills education experience avatarUrl location linkedin certifications languages projects resumeUrl"
      );

      // ✅ Also update parsed_data in Resume collection
      const parsedFields = {};

      jsonFields.forEach((key) => {
        if (updates[key]) parsedFields[key] = updates[key];
      });

      // ✅ Add LinkedIn to parsed_data as well (even if empty)
      parsedFields.linkedin = updates.linkedin || "";
      ...
      // ✅ Trigger AI Engine if resume was uploaded
      if (resumeFilePath) {
...
router.get("/profile", protect, isJobseeker, async (req, res) => {
  try {
    const user = await User.findById(req.user.id).select("bio avatarUrl");
    const resume = await Resume.findOne({ user_id: req.user.id })
      .sort({ updated_at: -1 });

    if (!resume || !resume.parsed_data) {
      return res.status(404).json({ msg: "Resume not parsed yet." });
    }
    ...
    res.json({
      ...
      summary: parsed.summary || parsed.summary_or_objective || "",
